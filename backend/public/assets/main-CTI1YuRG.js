import{t as R,c as H,a as A,s as b,f as h,b as T,d as z,v as u}from"./format-CGZL2wRF.js";function F(r,e){const t=R(r);return isNaN(e)?H(r,NaN):(t.setDate(t.getDate()+e),t)}class N{constructor(e){this.agendamentoData=e}async render(e){const t=document.getElementById(e);t&&(t.innerHTML=`
    ...
    `,this.setupEventListeners())}setupEventListeners(){const e=document.getElementById("generatePixBtn");e&&e.addEventListener("click",i=>{i.preventDefault(),this.generatePix()});const t=document.getElementById("copyPixBtn");t&&t.addEventListener("click",i=>{i.preventDefault(),this.copyPixCode()});const o=document.getElementById("simulatePixApprovedBtn");o&&o.addEventListener("click",i=>{i.preventDefault(),this.simulatePaymentApproved()})}simulatePaymentApproved(){console.log("🧪 Simulando pagamento aprovado..."),this.showPaymentSuccess()}async generatePix(){var t,o;const e=document.getElementById("generatePixBtn");e.disabled=!0,e.textContent="Gerando QR Code...";try{const i=await A.post("/payment/pix",{transaction_amount:this.agendamentoData.preco/100,description:`Vistoria ${this.getTipoVistoriaLabel(this.agendamentoData.tipo_vistoria)} - ${this.agendamentoData.protocolo}`,payer_email:this.agendamentoData.cliente_email,payer_first_name:this.agendamentoData.cliente_nome.split(" ")[0],payer_last_name:this.agendamentoData.cliente_nome.split(" ").slice(1).join(" ")||"Silva",payer_identification_type:"CPF",payer_identification_number:this.agendamentoData.cliente_cpf.replace(/\D/g,""),agendamento_id:this.agendamentoData.id});if(console.log("PIX Response:",i),!i.payment_id||!i.qr_code_base64)throw new Error("Resposta inválida do servidor");document.getElementById("pixQrCode").style.display="block",document.getElementById("qrCodeImage").innerHTML=`
        <img src="data:image/png;base64,${i.qr_code_base64}" alt="QR Code PIX" style="max-width: 300px; width: 100%;">
      `,document.getElementById("pixCode").value=i.qr_code,e.style.display="none",this.startPaymentPolling(i.payment_id)}catch(i){console.error("Error generating PIX:",i);const a=((o=(t=i.response)==null?void 0:t.data)==null?void 0:o.error)||i.message||"Erro ao gerar QR Code PIX";alert(a+`
Tente novamente.`),e.disabled=!1,e.textContent="Gerar QR Code PIX"}}copyPixCode(){document.getElementById("pixCode").select(),document.execCommand("copy");const t=document.getElementById("copyPixBtn"),o=t.textContent;t.textContent="Copiado!",setTimeout(()=>{t.textContent=o},2e3)}async startPaymentPolling(e){const t=async()=>{var n;try{const l=await A.get(`/payment/status/${e}`);return l.status==="approved"?(this.showPaymentSuccess(),!0):l.status==="rejected"||l.status==="cancelled"?(alert("Pagamento não foi aprovado. Tente novamente."),location.reload(),!0):!1}catch(l){return console.error("Error checking payment:",l),((n=l.response)==null?void 0:n.status)===404&&console.log("Payment not found yet, will retry..."),!1}};let i=0;const a=setInterval(async()=>{i++,(await t()||i>=100)&&clearInterval(a)},3e3)}showPaymentSuccess(){const e=document.querySelector(".payment-container");e.innerHTML=`
    ...
    `}showPaymentPending(){alert("Pagamento pendente. Aguardando confirmação do banco.")}getTipoVistoriaLabel(e){return{cautelar:"Vistoria Cautelar",transferencia:"Vistoria de Transferência",outros:"Outros Serviços"}[e]||e}formatDate(e){return new Date(e+"T00:00:00").toLocaleDateString("pt-BR")}}class O{constructor(e){this.container=document.getElementById(e),this.currentStep=1,this.formData={cliente:{},veiculo:{},tipo_vistoria:"",data:"",horario:"",endereco_vistoria:""},this.prices={},this.init()}async init(){var e,t;try{console.log("🔄 Initializing schedule form...");const o=sessionStorage.getItem("selectedService");o&&(this.formData.tipo_vistoria=o,console.log("📌 Pre-selected service:",o),sessionStorage.removeItem("selectedService")),this.prices=await b.getPrices(),console.log("✅ Prices loaded:",this.prices),this.render(),this.attachEventListeners()}catch(o){console.error("❌ Error initializing form:",o),console.error("Error details:",o.response||o.message);const i=((t=(e=o.response)==null?void 0:e.data)==null?void 0:t.error)||o.message||"Erro desconhecido";this.container.innerHTML=`
        <div class="alert alert-error" style="padding: 20px; background: #fee; border: 2px solid #f00; border-radius: 8px; margin: 20px 0;">
          ...
          <p style="font-size: 14px; color: #666;"><strong>Detalhes:</strong> ${i}</p>
        ...
        <div class="form-group">
          ...
          <select id="tipo_vistoria" name="tipo_vistoria" required>
            ...
            <option value="cautelar" ${this.formData.tipo_vistoria==="cautelar"?"selected":""}>
              Vistoria Cautelar - ${h.currency(((e=this.prices.cautelar)==null?void 0:e.valor)||15e3)}
            </option>
            <option value="transferencia" ${this.formData.tipo_vistoria==="transferencia"?"selected":""}>
              Vistoria Transferência - ${h.currency(((t=this.prices.transferencia)==null?void 0:t.valor)||12e3)}
            </option>
            <option value="outros" ${this.formData.tipo_vistoria==="outros"?"selected":""}>
              Outros - ${h.currency(((o=this.prices.outros)==null?void 0:o.valor)||1e4)}
            </option>
          </select>
      ...
      <div class="form-step ${this.currentStep===3?"active":""}" data-step="3">
        ...
        <div class="form-group">
          ...
          <input type="date" id="data" name="data" required
                 min="${T(new Date,"yyyy-MM-dd")}"
                 max="${T(F(new Date,30),"yyyy-MM-dd")}"
    ...
    `}attachEventListeners(){document.querySelectorAll("[data-mask]").forEach(o=>{o.addEventListener("input",i=>{z(i.target,i.target.dataset.mask)})}),document.querySelectorAll("[data-next]").forEach(o=>{o.addEventListener("click",i=>{const a=parseInt(i.target.dataset.next);this.validateStep(a)&&(this.saveStepData(a),this.nextStep(),typeof fbq<"u"&&fbq("track","Lead",{content_name:`Step ${a} Completed`,content_category:"Agendamento"}))})}),document.querySelectorAll("[data-prev]").forEach(o=>{o.addEventListener("click",()=>{this.prevStep()})});const e=document.getElementById("data");e&&e.addEventListener("change",async o=>{await this.loadTimeSlots(o.target.value)});const t=document.getElementById("confirmAndPayBtn");t&&t.addEventListener("click",async()=>{await this.createAppointmentAndPay()})}validateStep(e){const o=document.querySelector(`[data-step="${e}"]`).querySelectorAll("input[required], select[required]");let i=!0;return o.forEach(a=>{const n=a.closest(".form-group");if(n.classList.remove("error"),!u.required(a.value)){i=!1,n.classList.add("error");return}a.name==="cpf"&&!u.cpf(a.value)&&(i=!1,n.classList.add("error")),a.name==="email"&&!u.email(a.value)&&(i=!1,n.classList.add("error")),a.name==="telefone"&&!u.phone(a.value)&&(i=!1,n.classList.add("error")),a.name==="placa"&&!u.placa(a.value)&&(i=!1,n.classList.add("error")),a.name==="ano"&&!u.ano(a.value)&&(i=!1,n.classList.add("error"))}),e===3&&!this.formData.horario&&(document.getElementById("timeSlotsContainer").closest(".form-group").classList.add("error"),i=!1),i}saveStepData(e){e===1?this.formData.cliente={nome:document.getElementById("nome").value,cpf:document.getElementById("cpf").value,telefone:document.getElementById("telefone").value,email:document.getElementById("email").value}:e===2?(this.formData.veiculo={placa:document.getElementById("placa").value.toUpperCase(),marca:document.getElementById("marca").value,modelo:document.getElementById("modelo").value,ano:parseInt(document.getElementById("ano").value),chassi:document.getElementById("chassi").value||null},this.formData.tipo_vistoria=document.getElementById("tipo_vistoria").value,this.formData.endereco_vistoria=document.getElementById("endereco_vistoria").value):e===3&&(this.formData.data=document.getElementById("data").value)}async loadTimeSlots(e){const t=document.getElementById("timeSlotsContainer");t.innerHTML='<div class="spinner"></div>';try{const o=await b.getAvailableSlots(e);console.log("📅 Response from getAvailableSlots:",o),console.log("📅 Response type:",typeof o),console.log("📅 Is Array?:",Array.isArray(o));const i=Array.isArray(o)?o:o.slots||[];if(console.log("📅 Slots after normalization:",i),!Array.isArray(i)||i.length===0){t.innerHTML='<p style="text-align: center; color: #666;">Nenhum horário disponível para esta data</p>';return}t.innerHTML=`
      ...
      `,t.querySelectorAll(".time-slot:not(.unavailable)").forEach(a=>{a.addEventListener("click",()=>{t.querySelectorAll(".time-slot").forEach(n=>n.classList.remove("selected")),a.classList.add("selected"),this.formData.horario=a.dataset.time,t.closest(".form-group").classList.remove("error")})})}catch(o){console.error("Error loading time slots:",o),t.innerHTML='<p style="text-align: center; color: #dc3545;">Erro ao carregar horários. Tente novamente.</p>'}}nextStep(){this.currentStep===3&&this.renderSummary(),this.currentStep++,this.render(),this.attachEventListeners(),this.container.scrollIntoView({behavior:"smooth"})}prevStep(){this.currentStep--,this.render(),this.attachEventListeners(),this.currentStep===3&&this.formData.data&&this.loadTimeSlots(this.formData.data),this.container.scrollIntoView({behavior:"smooth"})}renderSummary(){var a;const e=this.formData.tipo_vistoria,t=((a=this.prices[e])==null?void 0:a.valor)||0,o=new Date(this.formData.data+"T00:00:00").toLocaleDateString("pt-BR"),i=`
      ...
      <div class="summary-box">
        <div class="summary-item">
          ...
          <span>${h.currency(t)}</span>
        </div>
      </div>
    `;setTimeout(()=>{const n=document.getElementById("summaryContent");n&&(n.innerHTML=i)},0)}async createAppointmentAndPay(){var t;const e=document.getElementById("confirmAndPayBtn");e.disabled=!0,e.innerHTML='<div class="spinner" style="margin: 0 auto; width: 20px; height: 20px;"></div>',console.log("📋 Form Data being sent:",JSON.stringify(this.formData,null,2));try{const o=await b.createAppointment(this.formData);if(typeof fbq<"u"){const i=((t=this.prices[this.formData.tipo_vistoria])==null?void 0:t.valor)||0;fbq("track","InitiateCheckout",{value:i/100,currency:"BRL",content_name:"Vistoria Agendada",content_category:this.formData.tipo_vistoria})}this.appointmentResult=o,this.nextStep(),await this.showPayment(o)}catch(o){console.error("Error creating appointment:",o),alert("Erro ao criar agendamento. Por favor, tente novamente."),e.disabled=!1,e.textContent="Confirmar e Pagar"}}async showPayment(e){if(!document.getElementById("paymentContainer"))return;await new N(e).render("paymentContainer")}showSuccess(e){this.container.innerHTML=`
    ...
    `,this.container.scrollIntoView({behavior:"smooth"})}}let E,p;function X(){E=new YT.Player("heroVideoPlayer",{videoId:"h00jrLVISGo",playerVars:{autoplay:1,controls:0,showinfo:0,modestbranding:1,loop:1,fs:0,cc_load_policy:0,iv_load_policy:3,autohide:1,mute:1,playsinline:1,rel:0},events:{onReady:j,onStateChange:G}}),p=new YT.Player("bottomVideoPlayer",{videoId:"h00jrLVISGo",playerVars:{autoplay:0,controls:0,showinfo:0,modestbranding:1,loop:1,fs:0,cc_load_policy:0,iv_load_policy:3,autohide:1,mute:1,playsinline:1,rel:0},events:{onReady:Y,onStateChange:Q}})}function j(r){r.target.mute(),r.target.playVideo()}function G(r){r.data===YT.PlayerState.ENDED&&(E.seekTo(0),E.playVideo())}function Y(r){r.target.mute()}function Q(r){r.data===YT.PlayerState.ENDED&&(p.seekTo(0),p.playVideo())}if(!window.YT){const r=document.createElement("script");r.src="https://www.youtube.com/iframe_api";const e=document.getElementsByTagName("script")[0];e.parentNode.insertBefore(r,e)}window.onYouTubeIframeAPIReady=X;document.addEventListener("DOMContentLoaded",async()=>{console.log("🚀 DOM Content Loaded - Initializing app..."),new O("scheduleApp"),await K(),setTimeout(B,100),W(),J(),U(),document.querySelectorAll('a[href^="#"]').forEach(r=>{r.addEventListener("click",function(e){e.preventDefault();const t=document.querySelector(this.getAttribute("href"));t&&t.scrollIntoView({behavior:"smooth",block:"start"})})}),console.log("✅ App initialization complete")});function B(){const r={root:null,rootMargin:"0px",threshold:.1},e=new IntersectionObserver(t=>{t.forEach(o=>{o.isIntersecting&&o.target.classList.add("active")})},r);document.querySelectorAll(".reveal, .reveal-up, .reveal-left, .reveal-right, .reveal-scale").forEach(t=>{e.observe(t)}),document.querySelectorAll(".stagger-container").forEach(t=>{t.querySelectorAll(".stagger-item").forEach((i,a)=>{setTimeout(()=>{new IntersectionObserver(l=>{l.forEach(y=>{y.isIntersecting&&y.target.classList.add("active")})},r).observe(i)},a*100)})})}function W(){const r=document.getElementById("siteHeader"),e=document.getElementById("mobileMenuToggle"),t=document.querySelector(".main-nav");window.addEventListener("scroll",()=>{window.scrollY>10?r.classList.add("scrolled"):r.classList.remove("scrolled")}),e&&e.addEventListener("click",()=>{e.classList.toggle("active"),t.classList.toggle("active")});const o=document.querySelectorAll("section[id]"),i=document.querySelectorAll(".nav-link");window.addEventListener("scroll",()=>{let a="";o.forEach(n=>{const l=n.offsetTop;n.clientHeight,scrollY>=l-200&&(a=n.getAttribute("id"))}),i.forEach(n=>{n.classList.remove("active"),n.getAttribute("href")===`#${a}`&&n.classList.add("active")})})}function U(){const r=document.getElementById("bottomVideoBg"),e=document.getElementById("agendamento");if(!r||!e)return;let t=!1;window.addEventListener("scroll",()=>{const o=e.offsetTop;window.scrollY+window.innerHeight>=o+200?r.classList.contains("visible")||(r.classList.add("visible"),p&&!t&&(p.playVideo(),t=!0)):r.classList.contains("visible")&&(r.classList.remove("visible"),p&&t&&(p.pauseVideo(),t=!1))})}function J(){const r=document.querySelector(".reviews-carousel-track"),e=document.querySelectorAll(".carousel-item"),t=document.querySelector(".carousel-prev"),o=document.querySelector(".carousel-next"),i=document.querySelector(".carousel-dots");if(!r||e.length===0)return;let a=0,n=P(),l=!1,y=0,f=0,x=0,S=0;r.style.userSelect="none",r.style.webkitUserSelect="none",r.style.msUserSelect="none";const v=Math.ceil(e.length/n);for(let s=0;s<v;s++){const d=document.createElement("button");d.classList.add("carousel-dot"),s===0&&d.classList.add("active"),d.addEventListener("click",()=>w(s)),i.appendChild(d)}function P(){return window.innerWidth<=768?1:window.innerWidth<=1024?2:3}function g(s=!0){const d=e[0].offsetWidth,m=parseInt(getComputedStyle(r).gap)||24,c=-(a*n*(d+m));s?r.style.transition="transform 0.3s ease-out":r.style.transition="none",r.style.transform=`translateX(${c}px)`,f=c,x=c,i.querySelectorAll(".carousel-dot").forEach((k,M)=>{k.classList.toggle("active",M===a)}),t.disabled=!1,o.disabled=!1}function w(s){a=s,g()}function V(){a>=v-1?a=0:a++,g()}function q(){a<=0?a=v-1:a--,g()}t.addEventListener("click",q),o.addEventListener("click",V),r.addEventListener("touchstart",$),r.addEventListener("touchend",D),r.addEventListener("touchmove",L),r.addEventListener("mousedown",$),r.addEventListener("mouseup",D),r.addEventListener("mouseleave",D),r.addEventListener("mousemove",L);function $(s){l=!0,y=I(s),S=requestAnimationFrame(_),r.style.cursor="grabbing"}function D(){l=!1,cancelAnimationFrame(S),r.style.cursor="grab";const s=f-x;s<-100&&a<v-1?a++:s>100&&a>0?a--:s<-100&&a>=v-1?a=0:s>100&&a<=0&&(a=v-1),g()}function L(s){if(l){const d=I(s);f=x+d-y}}function I(s){return s.type.includes("mouse")?s.pageX:s.touches[0].clientX}function _(){l&&(r.style.transition="none",r.style.transform=`translateX(${f}px)`,requestAnimationFrame(_))}e.forEach(s=>{s.querySelectorAll("img").forEach(m=>{m.addEventListener("dragstart",c=>c.preventDefault())})});let C;window.addEventListener("resize",()=>{clearTimeout(C),C=setTimeout(()=>{const s=P();if(s!==n){n=s,a=0,i.innerHTML="";const d=Math.ceil(e.length/n);for(let m=0;m<d;m++){const c=document.createElement("button");c.classList.add("carousel-dot"),m===0&&c.classList.add("active"),c.addEventListener("click",()=>w(m)),i.appendChild(c)}g()}},250)}),r.style.cursor="grab",g()}async function K(){try{const r=await b.getPrices(),e=document.getElementById("pricingGrid");if(!e)return;e.innerHTML=`
      <div class="pricing-card featured stagger-item">
        <div class="discount-badge">Mais Popular</div>
        <h3>Vistoria Cautelar</h3>
        <div class="price">${r.cautelar.valorFormatado}</div>
      ...
      <div class="pricing-card stagger-item">
        ...
        <div class="price">${r.transferencia.valorFormatado}</div>
      ...
      <div class="pricing-card stagger-item">
        ...
        <div class="price">A partir de ${r.outros.valorFormatado}</div>
    ...
    `,setTimeout(()=>{document.querySelectorAll(".btn-select-service").forEach(t=>{t.addEventListener("click",o=>{o.preventDefault();const i=t.dataset.service;sessionStorage.setItem("selectedService",i);const a=document.getElementById("tipo_vistoria");a&&(a.value=i,a.dispatchEvent(new Event("change",{bubbles:!0})));const n=document.getElementById("agendamento");n&&n.scrollIntoView({behavior:"smooth",block:"start"})})})},150),setTimeout(B,100)}catch(r){console.error("Error loading pricing:",r)}}
